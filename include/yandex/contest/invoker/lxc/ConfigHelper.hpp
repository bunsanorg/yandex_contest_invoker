...
void output(std::ostream &out, const std::string &key, const T &value) {
  if (key.find('=') != std::string::npos)
    BOOST_THROW_EXCEPTION(ConfigError()
                          << Error::message(
                                 "'=' character is prohibited in config key")
                          << ConfigError::key(key));
  ...
  if (line.find('\n') != std::string::npos)
    BOOST_THROW_EXCEPTION(ConfigError()
                          ...
                          << ConfigError::key(key) << ConfigError::line(line));
  out << line << std::endl;
}

template <typename T>
void optionalOutput(std::ostream &out, const std::string &key,
                    const boost::optional<T> &value) {
  if (value) output(out, key, value.get());
}

template <typename T>
void override_patch(boost::optional<T> &this_,
                    const boost::optional<T> &patch) {
  if (patch) this_ = patch;
}

template <typename T>
void recursive_patch(boost::optional<T> &this_,
                     const boost::optional<T> &patch) {
  if (patch) {
    if (this_) {
...
#define BOOST_OPTIONAL_OVERRIDE_PATCH(FIELD)                            \
  ::yandex::contest::invoker::lxc::config_helper::override_patch(FIELD, \
                                                                 config.FIELD)
#define BOOST_OPTIONAL_RECURSIVE_PATCH(FIELD)                      \
  ::yandex::contest::invoker::lxc::config_helper::recursive_patch( \
      FIELD, config.FIELD)
