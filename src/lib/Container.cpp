...
ContainerPointer Container::create(const ContainerConfig &config) {
  ...
  do {
    path = boost::filesystem::unique_path(config.containersDir /
           ...
           !(newDirectoryCreated = boost::filesystem::create_directory(path)));
  if (!newDirectoryCreated) {
    STREAM_ERROR << "New container was not created due to name collisions, "
                 << "throwing an exception.";
    BOOST_THROW_EXCEPTION(
        Error() << Error::message("Unable to create unique container"));
  }
  // create Lxc
  ...
  try {
    ...
    lxcPtr.reset(
        new lxc::Lxc(path.filename().string(), path, config.lxcConfig));
  } catch (...) {
...
ContainerPointer Container::create() { return create(ContainerConfig()); }
...
Filesystem &Container::filesystem() { return filesystem_; }

ProcessGroupPointer Container::createProcessGroup() {
  return ProcessGroup::create(ContainerPointer(this));
}

const ProcessGroup::DefaultSettings &Container::processGroupDefaultSettings()
    const {
...
void Container::stop() { lxcPtr_->stop(); }
