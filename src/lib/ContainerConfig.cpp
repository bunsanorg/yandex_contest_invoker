...
template <>
struct translator_between<std::string,
                          yandex::contest::system::unistd::MountEntry> {
  struct type {
    using internal_type = std::string;
    using external_type = yandex::contest::system::unistd::MountEntry;
...
namespace {
constexpr unsigned getWordSize() { return 8 * sizeof(void *); }

constexpr lxc::Config::Arch getArch() {
  static_assert(getWordSize() == 32 || getWordSize() == 64,
                "Unknown word size.");
  return getWordSize() == 32 ? lxc::Config::Arch::x86
...
lxc::MountConfig getLxcMountConfig() {
  ...
  const auto try_add = [&st](const boost::filesystem::path &path) {
    if (boost::filesystem::is_directory(path)) {
      st.entries->push_back(system::unistd::MountEntry::bindRO(path, path));
    }
  };
  try_add("/bin");
  try_add("/etc");
  ...
  for (const system::cgroup::HierarchyInfo &hierarchy :
    ...
    if (hierarchy.mountpoint) {
      st.entries->push_back(system::unistd::MountEntry::bind(
          *hierarchy.mountpoint, *hierarchy.mountpoint));
    }
  }
  return st;
}
...
filesystem::CreateFile getCharDevice(const boost::filesystem::path &path,
                                     const mode_t mode, const int major,
                                     const int minor) {
...
filesystem::Config getFilesystemConfig() {
  ...
  st.createFiles = {
      getDirectory("/dev", 0555), getCharDevice("/dev/null", 0666, 1, 3),
      getCharDevice("/dev/zero", 0666, 1, 5),
      getCharDevice("/dev/random", 0666, 1, 8),
      getCharDevice("/dev/urandom", 0666, 1, 9),
      getCharDevice("/dev/full", 0666, 1, 7), getSymLink("/proc/fd", "/dev/fd"),
