...
#define YANDEX_CONTEST_NOTIFIER_SIGNAL_IMPL(LNAME, UNAME)                \
  void operator()(const UNAME::Event &event) {                           \
    ioService_.dispatch(                                                 \
        boost::bind(&Impl::dispatch##UNAME, shared_from_this(), event)); \
  }                                                                      \
  void dispatch##UNAME(const UNAME::Event &event) { LNAME##_(event); }   \
  UNAME::Signal LNAME##_;

#define YANDEX_CONTEST_NOTIFIER_SIGNAL(LNAME, UNAME)                  \
  Notifier::Connection Notifier::on##UNAME(const UNAME::Slot &slot) { \
...
class Notifier::Impl : public boost::enable_shared_from_this<Impl>,
 ...
 public:
  ...
  void close() {
    strand_.dispatch([this] { closed_ = true; });
 ...
 private:
  void read() {
    notifierConnection_.async_read(
        inboundEvent_,
        strand_.wrap(boost::bind(&Impl::handle_read, this,
 ...
 private:
  ...
  notifier::ObjectConnection<posix::stream_descriptor> notifierConnection_;
...
Notifier::Notifier(io_service &ioService, const int notifierFd)
  ...
  onEvent(Event::Slot(&Impl::dispatch, pimpl.get(), _1).track(pimpl));
}
...
void Notifier::start() { pimpl->start(); }

void Notifier::close() { pimpl->close(); }
