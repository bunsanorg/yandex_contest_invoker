...
const ProcessMeta &ProcessInfo::meta() const { return meta_; }

void ProcessInfo::setMeta(const ProcessMeta &meta) { meta_ = meta; }

std::size_t ProcessInfo::id() const { return meta_.id; }

void ProcessInfo::setId(const std::size_t id) { meta_.id = id; }

const std::string &ProcessInfo::name() const { return meta_.name; }

void ProcessInfo::setName(const std::string &name) { meta_.name = name; }

Pid ProcessInfo::pid() const { return pid_; }

void ProcessInfo::setPid(const Pid pid) { pid_ = pid; }
...
bool ProcessInfo::terminated() const { return terminated_.load(); }
...
void ProcessInfo::fillTimeUsage(process::ResourceUsage &resourceUsage) const {
...
void ProcessInfo::updateMaxMemoryUsageBytes(
  ...
  while (
      ...
      !maxMemoryUsageBytes_.compare_exchange_weak(expected, memoryUsageBytes))
    continue;
}
...
bool ProcessInfo::setMaxMemoryUsageBytesIfZero(
    const std::uint64_t memoryUsageBytes) {
  std::uint64_t expected = 0;
  return maxMemoryUsageBytes_.compare_exchange_strong(expected,
                                                      memoryUsageBytes);
}
