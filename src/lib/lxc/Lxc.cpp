...
Lxc::Lxc(const std::string &name, const boost::filesystem::path &dir,
         const Config &config)
    : name_(name),
  ...
  STREAM_DEBUG << "Loading LXC config for \"" << name_ << "\" at "
               << configPath_;
...
void Lxc::freeze() {
  ...
  if (result) {
    ...
    STREAM_ERROR << "Error while freezing \"" << name_ << "\" LXC: \""
                 << result.err << "\", exception is thrown.";
    BOOST_THROW_EXCEPTION(toUtilityError(result)
...
void Lxc::unfreeze() {
  ...
  if (result) {
    ...
    STREAM_ERROR << "Error while unfreezing \"" << name_ << "\" LXC: \""
                 << result.err << "\", exception is thrown.";
    BOOST_THROW_EXCEPTION(toUtilityError(result)
...
void Lxc::execute_(const Executor &executor,
...
void Lxc::stop() {
  ...
  if (state_ == State::FROZEN) {
    STREAM_INFO << "\"" << name_ << "\" LXC is " << state_ << ", "
  ...
  do {
    ...
    if (result) {
      STREAM_INFO << "\"" << name_ << "\" LXC was successfully stopped.";
      return;
      ...
      STREAM_ERROR << "Error while stopping \"" << name_ << "\" LXC: \""
                   << result.err << "\", "
                   << "exception is thrown.";
      BOOST_THROW_EXCEPTION(toUtilityError(result)
...
const boost::filesystem::path &Lxc::rootfs() const { return rootfs_; }
...
void Lxc::prepare(Config &config) {
  ...
  if (config.mount) {
    if (config.mount->fstab) {
      ...
      for (unistd::MountEntry &entry : fstab) prepare(entry);
...
system::execution::AsyncProcess::Options Lxc::transform(
  ...
  opts.arguments =
      system::execution::collect("lxc-execute", "-n", name_, "-f", configPath_,
                                 "--", options.executable.string(), arguments);
  return opts;
}

UtilityError Lxc::toUtilityError(
    const system::execution::Result &result) const {
