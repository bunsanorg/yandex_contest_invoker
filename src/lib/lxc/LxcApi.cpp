...
static container_ptr container_new(const char *const name,
                                   const char *const configPath) {
  BOOST_ASSERT(name);

  container_ptr ptr(::lxc_container_new(name, configPath), false);
  if (!ptr)
    BOOST_THROW_EXCEPTION(UnableToCreateContainerError()
