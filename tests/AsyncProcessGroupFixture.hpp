...
struct AsyncProcessGroupFixture {
  using PG = ya::AsyncProcessGroup;
  using PGR = yandex::contest::invoker::process_group::Result;
  using PR = yandex::contest::invoker::process::Result;
  using TMP = yandex::contest::Tempfile;
  ...
  const PGR &pgr() const { return result.processGroupResult; }
  ...
  void verifyPGR(const PGR::CompletionStatus completionStatus =
                     PGR::CompletionStatus::OK) const {
    BOOST_CHECK_EQUAL(result.processGroupResult.completionStatus,
                      completionStatus);
  }

  void verifyPR(const std::size_t i,
                const PR::CompletionStatus completionStatus =
                    PR::CompletionStatus::OK) const {
  ...
  void verifyPRExit(const std::size_t i, const int exitStatus = 0) const {
    ...
    if (pr(i).exitStatus) BOOST_CHECK_EQUAL(pr(i).exitStatus.get(), exitStatus);
    BOOST_CHECK(!pr(i).termSig);
  }

  void verifyPRSig(const std::size_t i, const int termSig = 9,
                   const PR::CompletionStatus completionStatus =
                       PR::CompletionStatus::ABNORMAL_EXIT) const {
    verifyPR(i, completionStatus);
    BOOST_CHECK(!pr(i).exitStatus);
    BOOST_CHECK(pr(i).termSig);
    if (pr(i).termSig) BOOST_CHECK_EQUAL(pr(i).termSig.get(), termSig);
  }
  ...
  const std::vector<PR> &pr() const { return result.processResults; }
