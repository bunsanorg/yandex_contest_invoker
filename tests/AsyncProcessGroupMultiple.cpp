...
BOOST_AUTO_TEST_CASE(send_verify) {
  ...
  p1.executable = "sh";
  p1.arguments = {"sh", "-ce",
                  "read text; test \"arbitrary text\" = \"$text\""};
...
struct SendRecvFixture : BenchmarkFixture {
  ...
  void benchmark(const boost::filesystem::path &client,
                 const boost::filesystem::path &echoServer) {
    ...
    BOOST_TEST_MESSAGE(
        "echoServer: "
        << 1000 * static_cast<double>(count) /
               std::chrono::duration_cast<std::chrono::milliseconds>(
                   pr(0).resourceUsage.userTimeUsage).count()
        << " messages per cpu second.");
    BOOST_TEST_MESSAGE(
        "client: " << 1000 * static_cast<double>(count) /
                          std::chrono::duration_cast<std::chrono::milliseconds>(
                              pr(1).resourceUsage.userTimeUsage).count()
                   << " messages per cpu second.");
  }
};
