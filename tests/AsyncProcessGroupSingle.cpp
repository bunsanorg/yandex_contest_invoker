...
BOOST_AUTO_TEST_CASE(currentPath) {
  ...
  verifyPGR();
  verifyPRExit(0);
  BOOST_CHECK_EQUAL(filesystem::read_data(output.path()),
                    process.currentPath.string());
}
...
BOOST_AUTO_TEST_CASE(currentPathNoX) {
  ...
  verifyPGR();
  verifyPRExit(0);
  BOOST_CHECK_EQUAL(process.currentPath.string(),
                    filesystem::read_data(output.path()));
}
...
BOOST_AUTO_TEST_CASE(busy_beaver) {
  ...
  task.resourceLimits.realTimeLimit =
      10 * std::max(cpuLimitResolution, decaSleepTime);
...
BOOST_AUTO_TEST_CASE(exec) {
  ...
  std::chrono::nanoseconds singleCpuUsage;
  {
    ...
    verifyPGR();
    verifyPRExit(0);
    BOOST_REQUIRE_EQUAL(result.processGroupResult.completionStatus,
                        PGR::CompletionStatus::OK);
  ...
  // but a lot of exec are not
  {
    ...
    const std::size_t execNumber2x =
        std::count(output.begin(), output.end(), '\n');
    BOOST_REQUIRE_GT(execNumber2x, 2 * 4);
    BOOST_TEST_MESSAGE("Exec number: " << static_cast<double>(execNumber2x) /
                                              2);
    ...
    std::chrono::nanoseconds estimatedMultipleCpuUsage =
        (singleCpuUsage * execNumber2x) / 2;
    BOOST_TEST_MESSAGE("Estimated multiple cpu usage: "
                       << estimatedMultipleCpuUsage.count() << "us");
    const double multipleCpuUsageError =
        std::fabs((multipleCpuUsage - estimatedMultipleCpuUsage).count()) /
        multipleCpuUsage.count();
    BOOST_TEST_MESSAGE(
        "Estimated multiple cpu usage error: " << multipleCpuUsageError);
...
BOOST_AUTO_TEST_CASE(forker) {
  ...
  const std::uint64_t output =
      boost::lexical_cast<std::uint64_t>(filesystem::read_data(tmpfile.path()));
...
BOOST_AUTO_TEST_CASE(default_) {
  const TMP output;
  process.executable = "sh";
  process.arguments = {"sh", "-ce",
                       "echo -n `id -ru`:`id -rg`:`id -u`:`id -g`"};
  process.descriptors[1] = PG::File(output.path());
  run();
  verifyPGR();
  verifyPRExit(0);
...
bool checkUnresolvedFdAliasError(const ya::ResultError &e) {
  return std::string(e.what()).find("UnresolvedFdAliasError") !=
         std::string::npos;
}
}  // namespace
...
bool checkInvalidTargetFdAliasError(const ya::ResultError &e) {
  return std::string(e.what()).find("InvalidTargetFdAliasError") !=
         std::string::npos;
}
}  // namespace
...
BOOST_AUTO_TEST_CASE(file_cache) {
  ...
  TMP tmpfile;
  process.arguments = {"env", "dd", "if=/dev/zero",
                       "of=" + tmpfile.path().string(), "bs=1M", "count=64"};
...
BOOST_AUTO_TEST_CASE(lost_child) {
  ...
  verifyPRExit(0);
  const pid_t pid =
      boost::lexical_cast<pid_t>(filesystem::read_data(tmpfile.path()));
