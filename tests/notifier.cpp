...
BOOST_FIXTURE_TEST_CASE(Notifier, NotifierFixture) {
  ...
  notifier.onEvent(
      [&](const Notifier::Event::Event & /*event*/) { ++eventNumber; });
  notifier.onError([&](const Notifier::Error::Event &event) {
    error = true;
    BOOST_CHECK_EQUAL(event.errorCode, boost::asio::error::eof);
  });
  notifier.onSpawn([&](const Notifier::Spawn::Event &event) {
    spawn = true;
    BOOST_CHECK_EQUAL(event.meta.id, 1);
    BOOST_CHECK_EQUAL(event.meta.name, "id for spawn");
  });
  notifier.onTermination([&](const Notifier::Termination::Event &event) {
    termination = true;
    ...
    BOOST_CHECK(  // note: chrono does not support operator<<()
        event.result.resourceUsage.timeUsage == std::chrono::seconds(2));
    BOOST_CHECK_EQUAL(event.result.resourceUsage.memoryUsageBytes, 1024);
  });
  notifier.start();

  const Notifier::Event::Event spawnEvent_(spawnEvent);
  const Notifier::Event::Event terminationEvent_(terminationEvent);
  oc.async_write(spawnEvent_, [&](const boost::system::error_code &ec) {
    BOOST_REQUIRE(!ec);
    oc.async_write(terminationEvent_, [&](const boost::system::error_code &ec) {
      BOOST_REQUIRE(!ec);
      oc.close();
    });
  });
...
BOOST_FIXTURE_TEST_CASE(NotifierClose, NotifierFixture) {
  ...
  notifier.onError([&](const Notifier::Error::Event &event) {
    error = true;
    BOOST_CHECK_EQUAL(event.errorCode, boost::asio::error::operation_aborted);
  });
  ...
  oc.async_write(spawnEvent, [&](const boost::system::error_code &ec) {
    BOOST_REQUIRE(!ec);
  });

  ioService.post([&] { notifier.close(); });
...
#define BOOST_TEST_LOCK \
  ...
  yan::QueuedEventWriter<Connection> writer(
      oc, [&](const boost::system::error_code &ec) {
        BOOST_TEST_LOCK;
  ...
  const auto checkRunning = [&](const std::size_t id) {
    const auto iter = running.find(id);
    if (iter != running.end() && iter->second == 0) running.erase(iter);
    ++runningEvents;
  };
  const auto addRunning = [&](const std::size_t id) {
  ...
  const auto delRunning = [&](const std::size_t id) {
    boost::lock_guard<boost::mutex> lk(runningLock);
    --running[id];
    checkRunning(id);
  };

  notifier.onSpawn([&](const yac::Notifier::Spawn::Event &event) {
    addRunning(event.meta.id);
  });
  notifier.onTermination([&](const yac::Notifier::Termination::Event &event) {
    delRunning(event.meta.id);
  });
  notifier.onError([&](const yac::Notifier::Error::Event &event) {
    BOOST_TEST_LOCK;
    BOOST_CHECK_EQUAL(event.errorCode, boost::asio::error::eof);
  });
  notifier.start();
  ...
  for (std::size_t i = 0; i < WORKERS; ++i)
    threads.create_thread([&, i]() {
      ...
      {
        BOOST_TEST_LOCK;
        BOOST_TEST_MESSAGE("Spawn worker " << i << " has finished.");
      }
  ...
  for (std::size_t i = 0; i < WORKERS; ++i)
    threads.create_thread([&, i]() {
      ...
      {
        BOOST_TEST_LOCK;
        BOOST_TEST_MESSAGE("Termination worker " << i << " has finished.");
      }
      close.count_down_and_wait();
    });
